-- SKL V8 CORREGIDO
-- Correcciones de estabilidad/sintaxis sobre SKL V7.
-- No incluye funciones de evasión de anti-cheat ni ban bypass.
...
if _GENV.SKL_V8_LOADED then return end
_GENV.SKL_V8_LOADED = true
...
local function buildMacroObject(name, actions)
    local cfg = ZH.Runtime.RecordConfig or {}
    ...
    return {
        ...
        -- Configuración de referencia de la macro.
        -- Se guarda como dato; no fuerza clics ni acciones en el juego.
        config = {
            Map = tostring(cfg.Map or ZH.State.SelectedMap or ""),
            Level = math.max(
                1,
                tonumber(cfg.Level or ZH.State.SelectedLevel) or 1
            ),
        },
...
local function loadMacro(name)
    ...
    if type(obj.config) == "table" then
        if type(obj.config.Map) == "string" and obj.config.Map ~= "" then
            ZH.State.SelectedMap = obj.config.Map
        end

        local level = tonumber(obj.config.Level)
        if level then
            ZH.State.SelectedLevel = math.max(1, math.floor(level))
            clampSelectedLevel()
        end
    end
...
local function startRecording()
    ...
    -- Congela la configuración de la macro al comenzar la grabación.
    ZH.Runtime.RecordConfig = {
        Map = tostring(ZH.State.SelectedMap or ""),
        Level = math.max(1, tonumber(ZH.State.SelectedLevel) or 1),
    }
...
local function stopAndSaveRecording()
    ...
    -- Cierra cualquier pausa pendiente antes de guardar.
    ZH.State.RecordingPauseStartedAt = 0
    ZH.State.RecordingPausedTotal = math.max(
        0,
        tonumber(ZH.State.RecordingPausedTotal) or 0
    )
...
-- Estado unificado del recorder para la interfaz.
ZH.UI.GetRecorderStatus = function()
    if ZH.State.IsRecordingPaused then
        return "PAUSED"
    elseif ZH.State.IsRecording then
        return "RECORDING"
...
--------------------------------------------------------------------------------
-- SKL V8 - MINIMIZAR / ABRIR VENTANA
--------------------------------------------------------------------------------
task.defer(function()
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
        ...
        for _, root in ipairs(roots) do
            for _, obj in ipairs(root:GetChildren()) do
                if obj:IsA("ScreenGui") then
                    local hasFrame = obj:FindFirstChildWhichIsA("Frame", true)
                    if hasFrame then
                        return obj, hasFrame
                    end
                end
    ...
    local screen, main = findRoot()
    if not screen or not main then return end

    if screen:GetAttribute("SKLMinimizeAdded") then return end
    screen:SetAttribute("SKLMinimizeAdded", true)

    local originalSize = main.Size
    local minimized = false

    local function isGuiObject(x)
        return x and x:IsA("GuiObject")
    end

    -- Botón minimizar/restaurar en la esquina superior derecha.
    local minBtn = Instance.new("TextButton")
    minBtn.Name = "SKL_MinimizeButton"
    minBtn.Size = UDim2.fromOffset(32, 26)
    ...
    -- Botón flotante para abrir/restaurar cuando está minimizado.
    local openBtn = Instance.new("TextButton")
    openBtn.Name = "SKL_OpenButton"
    ...
    local function setMinimized(value)
        ...
        if minimized then
            -- Oculta los elementos internos, conservando la ventana como barra.
            for _, child in ipairs(main:GetChildren()) do
                if isGuiObject(child) and child ~= minBtn then
                    child.Visible = false
                end
            end
